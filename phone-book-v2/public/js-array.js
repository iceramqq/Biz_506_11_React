 ...
 * 각 요소를 arr변수에 넘기고 내부의 코ㅗ드를 샐행해서
...
const mapArr = myarray.map((arr) => {
  // arr++ // map 함수내부에 myarray의 요소를 변경하는 코드는
  // 가급적 지양하라
  return arr + 1;
});
 ...
 * 새로운 객체를 만들면소 원본 객체를 전개하고
 * key: value  형태의 값을 추가했을때
 *
 * 업으면 새로운 key:value 형태의 값을 추가하여 객체를 생성한다
 */
